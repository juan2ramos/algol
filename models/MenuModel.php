...
class MenuModelo extends Model {
    ...
    private function menu($menuM) {
        ...
        $where = " nombre_menu = '$menuM'";

        
        $menui = $this->selectDB($tabla, 2, $datos, $where,$otros);
            ...
            foreach ($menu['parents'][$padre] as $itemId) {
                ...
                if (isset($menu['parents'][$itemId])) {
                    ...
                    if ($menu['items'][$itemId]['key'] == 'sinpermiso') {
                        ...
                        $urlPadre =  (empty($menu['items'][$itemId]['url']))?'#':$this->_config->get('BaseUrl') . DS . $menu['items'][$itemId]['url'];
                        $html .= "<li>\n  <a href='$urlPadre' " . $_item_style . " >" . ucwords($menu['items'][$itemId]['nombre']) . "</a> \n";
                        $html .= $this->construirMenu($itemId, $menu,$classUl);
                        $html .= "</li> \n";
                    }
                }
